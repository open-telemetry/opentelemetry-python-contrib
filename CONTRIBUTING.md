...
- Uses a consistent instrumentation scope name when obtaining tracers, meters, or loggers. The name should identify the instrumentation package, such as `opentelemetry.instrumentation.requests`, rather than the instrumented library, such as `requests`, or an internal module path. If instrumentation code obtains telemetry from helper modules, prefer a shared constant over the helper module's `__name__` so the instrumentation reports one canonical scope name.
