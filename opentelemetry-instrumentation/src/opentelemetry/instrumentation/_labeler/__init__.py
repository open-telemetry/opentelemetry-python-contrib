...
Custom attributes are merged by any instrumentors that use ``enrich_metric_attributes`` before their calls to report individual metrics recording, such as ``Histogram.record``. ``enrich_metric_attributes`` does not overwrite base attributes that exist at the same keys.
"""
