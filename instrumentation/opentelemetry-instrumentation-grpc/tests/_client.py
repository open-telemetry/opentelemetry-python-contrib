...
def simple_method(stub, error=False):
    request = Request(client_id=CLIENT_ID, request_data="error" if error else "data")
...
def client_streaming_method(stub, error=False):
    ...
    stub.ClientStreamingMethod(request_messages(), metadata=(("key", "value"),))


def server_streaming_method(stub, error=False):
    request = Request(client_id=CLIENT_ID, request_data="error" if error else "data")
    response_iterator = stub.ServerStreamingMethod(request, metadata=(("key", "value"),))
...
def bidirectional_streaming_method(stub, error=False):
    ...
    response_iterator = stub.BidirectionalStreamingMethod(request_messages(), metadata=(("key", "value"),))
