...
class TestAioClientInterceptorMetrics(TestBase, IsolatedAsyncioTestCase):
    def setUp(self):
        super().setUp()
    ...
    async def test_unary_call_records_duration_metric(self):
        """Aio client RPC produces an rpc.client.call.duration histogram."""
    ...
    async def test_error_call_records_status_in_metric(self):
        """Aio client metric records the gRPC error status code on failure."""
        channel = grpc.aio.insecure_channel("localhost:25565")
        stub = test_server_pb2_grpc.GRPCTestServerStub(channel)
    ...
    async def test_server_streaming_records_duration_metric(self):
        """Aio client metric is recorded for a server-streaming RPC."""
        channel = grpc.aio.insecure_channel("localhost:25565")
        stub = test_server_pb2_grpc.GRPCTestServerStub(channel)
