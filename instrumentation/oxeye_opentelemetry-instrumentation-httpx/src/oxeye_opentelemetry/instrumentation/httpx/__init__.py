...
from oxeye_opentelemetry import context
from oxeye_opentelemetry.instrumentation.httpx.package import _instruments
from oxeye_opentelemetry.instrumentation.httpx.version import __version__
from oxeye_opentelemetry.instrumentation.instrumentor import BaseInstrumentor
from oxeye_opentelemetry.instrumentation.utils import (
    http_status_to_status_code,
    unwrap,
)
from oxeye_opentelemetry.propagate import inject
from oxeye_opentelemetry.semconv.trace import SpanAttributes
from oxeye_opentelemetry.trace import SpanKind, Tracer, TracerProvider, get_tracer
from oxeye_opentelemetry.trace.span import Span
from oxeye_opentelemetry.trace.status import Status
...
class Syncoxeye_opentelemetryTransport(httpx.BaseTransport):
...
class Asyncoxeye_opentelemetryTransport(httpx.AsyncBaseTransport):
...
class _InstrumentedClient(httpx.Client):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._transport = Syncoxeye_opentelemetryTransport(
            self._transport,
...
class _InstrumentedAsyncClient(httpx.AsyncClient):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._transport = Asyncoxeye_opentelemetryTransport(
            self._transport,
...
class HTTPXClientInstrumentor(BaseInstrumentor):
    ...
    def instrument_client(
        ...
        if not hasattr(client, "_is_instrumented_by_oxeye_opentelemetry"):
            client._is_instrumented_by_oxeye_opentelemetry = False

        if not client._is_instrumented_by_oxeye_opentelemetry:
            if isinstance(client, httpx.Client):
                client._original_transport = client._transport
                transport = client._transport or httpx.HTTPTransport()
                client._transport = Syncoxeye_opentelemetryTransport(
                    transport,
                    tracer_provider=tracer_provider,
                    request_hook=request_hook,
                    response_hook=response_hook,
                )
                client._is_instrumented_by_oxeye_opentelemetry = True
            if isinstance(client, httpx.AsyncClient):
                transport = client._transport or httpx.AsyncHTTPTransport()
                client._transport = Asyncoxeye_opentelemetryTransport(
                    transport,
                    tracer_provider=tracer_provider,
                    request_hook=request_hook,
                    response_hook=response_hook,
