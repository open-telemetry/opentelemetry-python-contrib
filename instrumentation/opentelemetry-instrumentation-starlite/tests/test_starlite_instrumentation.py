...
import opentelemetry.instrumentation.starlite as otel_starlite
...
class TestStarliteManualInstrumentation(TestBase):
    def _create_app(self):
        app = self._create_starlite_app()
    ...
    def setUp(self):
        ...
        self.exclude_patch = patch(
            "opentelemetry.instrumentation.starlite._excluded_urls",
            get_excluded_urls("STARLETTE"),
        )
        self.exclude_patch.start()
        self._instrumentor = otel_starlite.StarliteInstrumentor()
    ...
    def test_basic_starlite_call(self):
    ...
    def test_starlite_route_attribute_added(self):
        """Ensure that starlite routes are used as the span name."""
    ...
    def test_starlite_excluded_urls(self):
        """Ensure that given starlite routes are excluded."""
        self._client.get("/healthzz")
        spans = self.memory_exporter.get_finished_spans()
        self.assertEqual(len(spans), 0)

    def test_starlite_metrics(self):
        self._client.get("/foobar")
        self._client.get("/foobar")
        self._client.get("/foobar")
...
class TestStarliteManualInstrumentationHooks(
    TestStarliteManualInstrumentation
):
...
class TestAutoInstrumentation(TestStarliteManualInstrumentation):
    ...
    def _create_app(self):
        ...
        return self._create_starlite_app()
...
class TestAutoInstrumentationHooks(TestStarliteManualInstrumentationHooks):
    """
...
class TestAutoInstrumentationLogic(unittest.TestCase):
    def test_instrumentation(self):
        ...
        """
        instrumentor = otel_starlite.StarliteInstrumentor()
        original = applications.Starlite
        instrumentor.instrument()
        try:
            instrumented = applications.Starlite
            self.assertIsNot(original, instrumented)
        finally:
            instrumentor.uninstrument()

        should_be_original = applications.Starlite
        self.assertIs(original, should_be_original)


class TestConditonalServerSpanCreation(TestStarliteManualInstrumentation):
    def test_mark_span_internal_in_presence_of_another_span(self):
        ...
        with tracer.start_as_current_span(
            ...
            starlite_span = spans[2]
            self.assertEqual(SpanKind.INTERNAL, starlite_span.kind)
            self.assertEqual(SpanKind.SERVER, parent_span.kind)
            self.assertEqual(
                parent_span.context.span_id, starlite_span.parent.span_id
            )


class TestBaseWithCustomHeaders(TestBase):
    def create_app(self):
        app = self.create_starlite_app()
