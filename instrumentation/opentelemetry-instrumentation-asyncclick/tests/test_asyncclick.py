...
class ClickTestCase(TestBase, IsolatedAsyncioTestCase):
    ...
    def test_uvicorn_cli_command_ignored(self):
        @asyncclick.command("uvicorn")
        async def command_uvicorn() -> None:
            pass

        result = run_asyncclick_command_test(command_uvicorn)
        self.assertEqual(result.exit_code, 0)

        self.assertFalse(self.memory_exporter.get_finished_spans())

    @pytest.mark.skipif(flask_cli is None, reason="requires flask")
    def test_flask_run_command_ignored(self) -> None:
