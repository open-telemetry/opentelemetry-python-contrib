...
.. code:: python
    ...
    from oxeye_opentelemetry.instrumentation.pyramid import PyramidInstrumentor
...
you need to add ``oxeye_opentelemetry.instrumentation.pyramid.trace_tween_factory`` explicity to the list,
...
.. code:: python
    ...
    settings = {
        'pyramid.tweens', 'oxeye_opentelemetry.instrumentation.pyramid.trace_tween_factory\\nyour_tween_no_1\\nyour_tween_no_2',
    }
...
from oxeye_opentelemetry.instrumentation.pyramid.callbacks import (
    SETTING_TRACE_ENABLED,
    TWEEN_NAME,
    trace_tween_factory,
)
from oxeye_opentelemetry.instrumentation.pyramid.package import _instruments
...
from oxeye_opentelemetry.trace import TracerProvider, get_tracer
...
def _traced_init(wrapped, instance, args, kwargs):
    ...
    wrapped(*args, **kwargs)
    instance.include("oxeye_opentelemetry.instrumentation.pyramid.callbacks")
...
class PyramidInstrumentor(BaseInstrumentor):
    ...
    def instrument_config(config):
        ...
        """
        config.include("oxeye_opentelemetry.instrumentation.pyramid.callbacks")
