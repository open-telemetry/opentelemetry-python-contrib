...
from opentelemetry.instrumentation.django.middleware.sqlcommenter_middleware import QueryWrapper
import django
from django import VERSION, conf
from django.http import HttpResponse
from django.test.utils import setup_test_environment, teardown_test_environment, override_settings
...
class TestMiddleware(TestBase, WsgiTestBase):
    @classmethod
    def setUpClass(cls):
        conf.settings.configure(SQLCOMMENTER_WITH_OPENTELEMETRY=True, SQLCOMMENTER_WITH_FRAMEWORK=False)
        super().setUpClass()
    ...
    def test_middleware_added(self, sqlcommenter_middleware):
        ...
        self.assertTrue(
            'opentelemetry.instrumentation.django.middleware.sqlcommenter_middleware.SqlCommenter' in django.conf.settings.MIDDLEWARE)
    ...
    def test_query_wrapper(self, trace_capture):
        ...
        qw_instance(execute_mock_obj, "Select 1", MagicMock("test"), MagicMock("test1"), MagicMock())
        output_sql = execute_mock_obj.call_args[0][0]
        self.assertEqual(output_sql,
                         "Select 1 /*controller='view',route='route',traceparent='%%2Atraceparent%%3D%%2700-000000000000000000000000deadbeef-000000000000beef-00'*/")
