...
import oxeye_opentelemetry.instrumentation.fastapi as otel_fastapi
...
class TestFastAPIManualInstrumentation(TestBase):
    ...
    def setUp(self):
        ...
        self.exclude_patch = patch(
            "oxeye_opentelemetry.instrumentation.fastapi._excluded_urls_from_env",
    ...
    def test_uninstrument_app(self):
        ...
        self.assertFalse(
            isinstance(
                self._app.user_middleware[0].cls, oxeye_opentelemetryMiddleware
            )
        )
        self._client = TestClient(self._app)
