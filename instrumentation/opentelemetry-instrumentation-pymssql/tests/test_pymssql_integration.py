...
class TestPyMSSQLIntegration(TestBase):
    ...
    def test_instrument_connection_capture_parameters_default(self, mock_instrument_connection):
        connection = Mock()
    ...
    @patch("opentelemetry.instrumentation.pymssql.dbapi.instrument_connection")
    def test_instrument_connection_capture_parameters_enabled(self, mock_instrument_connection):
        connection = Mock()
        PyMSSQLInstrumentor().instrument_connection(connection, capture_parameters=True)
        _, kwargs = mock_instrument_connection.call_args
        self.assertTrue(kwargs["capture_parameters"])
