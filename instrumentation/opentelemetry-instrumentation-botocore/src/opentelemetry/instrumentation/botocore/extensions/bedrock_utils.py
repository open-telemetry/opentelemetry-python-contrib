...
class ConverseStreamWrapper(BaseObjectProxy):
    ...
    def _process_event(self, event):
        ...
        if "contentBlockStart" in event:
            # {'contentBlockStart': {'start': {'toolUse': {'toolUseId': 'id', 'name': 'func_name'}},
            #  'contentBlockIndex': 1}}
        ...
        if "metadata" in event:
            # {'metadata': {'usage': {'inputTokens': 12, 'outputTokens': 15, 'totalTokens': 27},
            #  'metrics': {'latencyMs': 2980}}}
...
class InvokeModelWithResponseStreamWrapper(BaseObjectProxy):
    ...
    def _process_amazon_nova_chunk(self, chunk):
        ...
        if "contentBlockStart" in chunk:
            # {'contentBlockStart': {'start': {'toolUse': {'toolUseId': 'id', 'name': 'name'}},
            #  'contentBlockIndex': 31}}
            if self._record_message:
                self._message["content"].append(self._content_block)
        ...
        if "contentBlockDelta" in chunk:
            # {'contentBlockDelta': {'delta': {'text': "Hello"}, 'contentBlockIndex': 0}}
            # {'contentBlockDelta': {'delta': {'toolUse': {'input': '{"location":"San Francisco"}'}},
            #  'contentBlockIndex': 31}}
            if self._record_message:
    ...
    def _process_anthropic_claude_chunk(self, chunk):
        ...
        if message_type == "content_block_start":
            ...
            # {'type': 'content_block_start', 'index': 1, 'content_block': {'type': 'tool_use', 'id': 'id',
            #  'name': 'func_name', 'input': {}}}
            if self._record_message:
        ...
        if message_type == "message_stop":
            # {'type': 'message_stop', 'amazon-bedrock-invocationMetrics': {'inputTokenCount': 18,
            #  'outputTokenCount': 123, 'invocationLatency': 5250, 'firstByteLatency': 290}}
            if invocation_metrics := chunk.get("amazon-bedrock-invocationMetrics"):
                self._process_invocation_metrics(invocation_metrics)
