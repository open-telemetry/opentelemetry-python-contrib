...
def _add_create_attributes(span: Span, args: tuple[Any, ...]):
    ...
    # According to:
    # https://github.com/redis/redis-py/blob/master/redis/commands/search/commands.py#L155
    # schema is last argument for execute command
    try:
    ...
    field_attribute = ""
    # Schema in format:
    # [first_field_name, first_field_type, first_field_some_attribute1,
    #  first_field_some_attribute2, second_field_name, ...]
...
def _add_search_attributes(span: Span, response, args):
    ...
    #  index_of_second_returned_doc, second_doc(as a list) ...]
...
def _build_span_meta_data_for_pipeline(
    ...
    try:
        command_stack = instance.command_stack if hasattr(instance, "command_stack") else instance._command_stack
