...
def get_operation_name(hook_name, retry_count):
    if hook_name == "before_process_message":
        return "remoulade/process" if retry_count == 0 else f"remoulade/process(retry-{retry_count})"
    if hook_name == "before_enqueue":
        return "remoulade/send" if retry_count == 0 else f"remoulade/send(retry-{retry_count})"
    return ""
