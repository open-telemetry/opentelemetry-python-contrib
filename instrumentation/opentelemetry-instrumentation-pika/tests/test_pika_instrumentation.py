...
class TestPika(TestCase):
    ...
    @staticmethod
    def _consumer_callback(channel: object, method: object, properties: object, body: object) -> None:
        pass

    def test_instrument_api(self) -> None:
        instrumentation = PikaInstrumentor()
        instrumentation.instrument()
        self.assertTrue(isinstance(BlockingConnection.channel, BoundFunctionWrapper))
        self.assertTrue(isinstance(Connection.channel, BoundFunctionWrapper))
        self.assertTrue(isinstance(BaseConnection.channel, BoundFunctionWrapper))
        self.assertTrue(isinstance(_QueueConsumerGeneratorInfo.__init__, BoundFunctionWrapper))
        assert hasattr(instrumentation, "__opentelemetry_tracer_provider"), "Tracer not stored for the object!"
        instrumentation.uninstrument()
        self.assertFalse(isinstance(BlockingConnection.channel, BoundFunctionWrapper))
        self.assertFalse(isinstance(Connection.channel, BoundFunctionWrapper))
        self.assertFalse(isinstance(BaseConnection.channel, BoundFunctionWrapper))
        self.assertFalse(isinstance(_QueueConsumerGeneratorInfo.__init__, BoundFunctionWrapper))

    @mock.patch("opentelemetry.instrumentation.pika.PikaInstrumentor._instrument_channel_functions")
    @mock.patch("opentelemetry.instrumentation.pika.PikaInstrumentor._decorate_basic_consume")
    @mock.patch("opentelemetry.instrumentation.pika.PikaInstrumentor._instrument_channel_consumers")
    def test_instrument_blocking_channel(
        ...
        assert hasattr(self.blocking_channel, "_is_instrumented_by_opentelemetry"), (
    ...
    def test_instrument_channel(
        ...
        assert hasattr(self.channel, "_is_instrumented_by_opentelemetry"), "channel is not marked as instrumented!"
        instrument_channel_consumers.assert_called_once()
        instrument_basic_consume.assert_called_once()
        instrument_channel_functions.assert_called_once()

    @mock.patch("opentelemetry.instrumentation.pika.utils._decorate_callback")
    def test_instrument_consumers_on_blocking_channel(self, decorate_callback: mock.MagicMock) -> None:
        ...
        decorate_callback.assert_has_calls(calls=expected_decoration_calls, any_order=True)
        assert all(
            hasattr(
    ...
    @mock.patch("opentelemetry.instrumentation.pika.utils._decorate_callback")
    def test_instrument_consumers_on_channel(self, decorate_callback: mock.MagicMock) -> None:
        tracer = mock.MagicMock(spec=Tracer)
        callback_attr = PikaInstrumentor.CONSUMER_CALLBACK_ATTR
        expected_decoration_calls = [
            mock.call(getattr(value, callback_attr), tracer, key, dummy_callback)
    ...
    @mock.patch("opentelemetry.instrumentation.pika.utils._decorate_callback")
    def test_instrument_consumers_skips_already_decorated_callbacks(self, decorate_callback: mock.MagicMock) -> None:
        tracer = mock.MagicMock(spec=Tracer)
        callback_attr = PikaInstrumentor.CONSUMER_CALLBACK_ATTR
    ...
    def test_sequential_basic_consume_registrations_do_not_duplicate_spans(
        ...
        class FakeBlockingChannel:
            ...
            def basic_consume(self, queue: str, *args: object, **kwargs: object) -> str:
                consumer_callback = (
                ...
                self._consumer_infos[queue] = SimpleNamespace(**{callback_attr: consumer_callback})
                return queue
        ...
        self.assertEqual(
            Counter(span.name for span in spans if span.kind == SpanKind.CONSUMER),
            Counter({"q1 receive": 1}),
        )
        self.assertEqual(callback_calls, [(channel, method, properties, b"hello")])
        channel.basic_ack.assert_called_once_with(1)

    @mock.patch("opentelemetry.instrumentation.pika.utils._decorate_basic_publish")
    def test_instrument_basic_publish_on_blocking_channel(self, decorate_basic_publish: mock.MagicMock) -> None:
        tracer = mock.MagicMock(spec=Tracer)
    ...
    @mock.patch("opentelemetry.instrumentation.pika.utils._decorate_basic_publish")
    def test_instrument_basic_publish_on_channel(self, decorate_basic_publish: mock.MagicMock) -> None:
        tracer = mock.MagicMock(spec=Tracer)
    ...
    def test_instrument_queue_consumer_generator(self) -> None:
        instrumentation = PikaInstrumentor()
        instrumentation.instrument()
        generator_info = _QueueConsumerGeneratorInfo(params=("queue", False, False), consumer_tag="tag")
        self.assertTrue(isinstance(generator_info.pending_events, ReadyMessagesDequeProxy))
        instrumentation.uninstrument()
        generator_info = _QueueConsumerGeneratorInfo(params=("queue", False, False), consumer_tag="tag")
        self.assertFalse(isinstance(generator_info.pending_events, ReadyMessagesDequeProxy))

    def test_uninstrument_blocking_channel_functions(self) -> None:
        original_function = self.blocking_channel.basic_publish
        self.blocking_channel.basic_publish = mock.MagicMock()
        self.blocking_channel.basic_publish._original_function = original_function
        PikaInstrumentor._uninstrument_channel_functions(self.blocking_channel)
        self.assertEqual(self.blocking_channel.basic_publish, original_function)
    ...
    def test_consumer_callback_attribute_name(self) -> None:
        with mock.patch("pika.__version__", "1.0.0"):
            self.assertEqual(_consumer_callback_attribute_name(), "on_message_callback")
        with mock.patch("pika.__version__", "0.12.0"):
            self.assertEqual(_consumer_callback_attribute_name(), "consumer_cb")
