...
def collect_custom_request_headers_attributes(environ):
    ...
    s = SanitizeValue(
        [_CARRIER_KEY_PREFIX + i.replace("-", "_") for i in sanitized_fields]
    )
    ...
    if custom_request_headers_name:
        custom_request_headers_regex_compiled = re.compile(
            "|".join(
                "^" + _CARRIER_KEY_PREFIX + i.replace("-", "_") + "$"
                for i in custom_request_headers_name
            ),
            re.IGNORECASE,
        )

        for header_name in list(
            filter(custom_request_headers_regex_compiled.match, environ.keys())
        ):
            header_values = environ.get(header_name)
            if header_values:
                key = normalise_request_header_name(
                    header_name[_CARRIER_KEY_PREFIX_LEN:]
                )
                attributes[key] = [
