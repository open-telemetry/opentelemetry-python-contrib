...
class TestSqlalchemyInstrumentation(TestBase):
    ...
    def test_capture_parameters_off_by_default(self):
        ...
        self.assertFalse(any(key.startswith(f"{DB_QUERY_PARAMETER_TEMPLATE}.") for key in query_span.attributes))
    ...
    def test_capture_parameters_executemany(self):
        ...
        spans = self.memory_exporter.get_finished_spans()
        for span in spans:
            self.assertFalse(any(key.startswith(f"{DB_QUERY_PARAMETER_TEMPLATE}.") for key in span.attributes))
    ...
    def _get_query_span(self):
        ...
        for span in spans:
            if span.kind is trace.SpanKind.CLIENT and (
                DB_SYSTEM in span.attributes or DB_SYSTEM_NAME in span.attributes
            ):
