...
from opentelemetry.instrumentation.pubsub.utils import (
...
class TestUtils(TestCase):
    ...
    @mock.patch(
        "opentelemetry.instrumentation.pubsub.utils.KafkaPropertiesExtractor.extract_bootstrap_servers"
    )
    @mock.patch(
        "opentelemetry.instrumentation.pubsub.utils.KafkaPropertiesExtractor.extract_send_partition"
    )
    @mock.patch("opentelemetry.instrumentation.pubsub.utils._enrich_span")
    ...
    @mock.patch(
        "opentelemetry.instrumentation.pubsub.utils._create_consumer_span"
    )
    @mock.patch(
        "opentelemetry.instrumentation.pubsub.utils.KafkaPropertiesExtractor.extract_bootstrap_servers"
    )
    ...
    @mock.patch(
        "opentelemetry.instrumentation.pubsub.utils.KafkaPropertiesExtractor"
    )
