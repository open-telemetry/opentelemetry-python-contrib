...
from libmc import __version__ as libmc_package_version
from libmc.exceptions import (
    MemcacheClientError,
...
from opentelemetry.instrumentation.libmc import libmcInstrumentor
...
libmc_version = get_package_version.parse(libmc_package_version)

# In libmc versions greater than 2, set_multi, set_many and delete_multi
# now use a batched call
# https://github.com/pinterest/libmc/blob/master/ChangeLog.rst#new-in-version-200  # noqa
libmc_version_lt_2 = libmc_version < get_package_version.parse(
    "2.0.0"
)

# In libmc versions greater than 3.4.1, the stats command no
# longer sends trailing whitespace in the command
# https://github.com/pinterest/libmc/blob/master/ChangeLog.rst#new-in-version-342  # noqa
libmc_version_gt_341 = libmc_version > get_package_version.parse(
    "3.4.1"
)


class libmcClientTestCase(
    TestBase
):  # pylint: disable=too-many-public-methods
    """Tests for a patched libmc.client.base.Client."""

    def setUp(self):
        super().setUp()
        libmcInstrumentor().instrument()
    ...
    def tearDown(self):
        super().tearDown()
        libmcInstrumentor().uninstrument()

    def make_client(self, mock_socket_values, **kwargs):
        # pylint: disable=attribute-defined-outside-init
        self.client = libmc.client.base.Client(
    ...
    def test_set_multi_success(self):
        ...
        # In libmc versions greater than 2, set_multi now uses a batched call
        # https://github.com/pinterest/libmc/blob/master/ChangeLog.rst#new-in-version-200  # noqa
        if libmc_version_lt_2:
            self.assertTrue(result)
    ...
    def test_delete_many_found(self):
        ...
        # In libmc versions greater than 2, delete_many now uses a batched call
        # https://github.com/pinterest/libmc/blob/master/ChangeLog.rst#new-in-version-200  # noqa
        if libmc_version_lt_2:
            self.check_spans(
    ...
    def test_set_many_success(self):
        ...
        # In libmc versions greater than 2, set_many now uses a batched call
        # https://github.com/pinterest/libmc/blob/master/ChangeLog.rst#new-in-version-200  # noqa
        if libmc_version_lt_2:
            self.assertTrue(result)
    ...
    def test_no_op_tracer_provider(self):
        ...
        libmcInstrumentor().instrument(tracer_provider=tracer_provider)
...
class libmcHashClientTestCase(TestBase):
    """Tests for a patched libmc.client.hash.HashClient."""
    ...
    def make_client_pool(
        ...
        mock_client = libmc.client.base.Client(
            hostname, serializer=serializer, **kwargs
        )
        mock_client.sock = MockSocket(mock_socket_values)
        client = libmc.client.base.PooledClient(
            hostname, serializer=serializer
        )
        client.client_pool = libmc.pool.ObjectPool(lambda: mock_client)
        return mock_client
    ...
    def make_client(self, *mock_socket_values, **kwargs):
        ...
        from libmc.client.hash import HashClient
