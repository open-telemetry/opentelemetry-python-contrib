...
def test_chat_completion_with_content(span_exporter, log_exporter, openai_client, instrument_with_content, vcr):
    latest_experimental_enabled = is_experimental_mode()
    print(
        f"latest_experimental_enabled={latest_experimental_enabled}, "
        f"ENV_VAR={os.getenv('OTEL_SEMCONV_STABILITY_OPT_IN')}"
    )
...
def test_chat_completion_no_content(span_exporter, log_exporter, openai_client, instrument_no_content, vcr):
    latest_experimental_enabled = is_experimental_mode()
    with vcr.use_cassette("test_chat_completion_no_content.yaml"):
        response = openai_client.chat.completions.create(messages=USER_ONLY_PROMPT, model=DEFAULT_MODEL, stream=False)
...
def test_chat_completion_bad_endpoint(span_exporter, metric_reader, instrument_no_content, vcr):
    latest_experimental_enabled = is_experimental_mode()
...
def test_chat_completion_404(span_exporter, openai_client, metric_reader, instrument_no_content, vcr):
    latest_experimental_enabled = is_experimental_mode()
...
def test_chat_completion_api_exception_propagates(openai_client, instrument_no_content, vcr):
    with vcr.use_cassette("test_chat_completion_404.yaml"):
        with pytest.raises(NotFoundError):
            openai_client.chat.completions.create(
                messages=USER_ONLY_PROMPT,
...
def test_chat_completion_extra_params(span_exporter, openai_client, instrument_no_content, vcr):
    latest_experimental_enabled = is_experimental_mode()
    ...
    assert spans[0].attributes[GenAIAttributes.GEN_AI_REQUEST_STOP_SEQUENCES] == ("full", "stop")
    assert spans[0].attributes[GenAIAttributes.GEN_AI_REQUEST_CHOICE_COUNT] == 2


def test_chat_completion_n_1_is_not_reported(span_exporter, openai_client, instrument_no_content, vcr):
    latest_experimental_enabled = is_experimental_mode()
    ...
    assert GenAIAttributes.GEN_AI_REQUEST_CHOICE_COUNT not in spans[0].attributes


def test_chat_completion_handle_stop_sequences_as_string(span_exporter, openai_client, instrument_no_content, vcr):
    latest_experimental_enabled = is_experimental_mode()
    ...
    assert spans[0].attributes[GenAIAttributes.GEN_AI_REQUEST_STOP_SEQUENCES] == ("stop",)


def test_chat_completion_multiple_choices(span_exporter, log_exporter, openai_client, instrument_with_content, vcr):
    latest_experimental_enabled = is_experimental_mode()
...
def test_chat_completion_with_raw_response(span_exporter, log_exporter, openai_client, instrument_with_content, vcr):
    latest_experimental_enabled = is_experimental_mode()
...
def test_chat_completion_tool_calls_no_content(span_exporter, log_exporter, openai_client, instrument_no_content, vcr):
...
def chat_completion_tool_call(
    ...
    response_1 = openai_client.chat.completions.create(messages=messages_value, model=DEFAULT_MODEL)
    ...
    if latest_experimental_enabled:
        if not expect_content:
            ...
            assert_messages_attribute(spans[0].attributes["gen_ai.output.messages"], first_output)
            ...
            assert_messages_attribute(spans[1].attributes["gen_ai.input.messages"], second_input)

            assert_messages_attribute(
                spans[1].attributes["gen_ai.output.messages"],
                format_simple_expected_output_message(response_1.choices[0].message.content),
            )
    else:
        assert len(logs) == 9  # 3 logs for first completion, 6 for second

        # call one
        system_message = {"content": messages_value[0]["content"]} if expect_content else None
        assert_message_in_logs(logs[0], "gen_ai.system.message", system_message, spans[0])

        user_message = {"content": messages_value[1]["content"]} if expect_content else None
        assert_message_in_logs(logs[1], "gen_ai.user.message", user_message, spans[0])

        function_call_0 = {"name": "get_current_weather"}
        function_call_1 = {"name": "get_current_weather"}
        if expect_content:
            function_call_0["arguments"] = (
                response_0.choices[0].message.tool_calls[0].function.arguments.replace("\n", "")
            )
            function_call_1["arguments"] = (
                response_0.choices[0].message.tool_calls[1].function.arguments.replace("\n", "")
            )
        ...
        # call two
        system_message = {"content": messages_value[0]["content"]} if expect_content else None
        assert_message_in_logs(logs[3], "gen_ai.system.message", system_message, spans[1])

        user_message = {"content": messages_value[1]["content"]} if expect_content else None
        assert_message_in_logs(logs[4], "gen_ai.user.message", user_message, spans[1])
        ...
        assert_message_in_logs(logs[5], "gen_ai.assistant.message", assistant_tool_call, spans[1])
        ...
        assert_message_in_logs(logs[6], "gen_ai.tool.message", tool_message_0, spans[1])
        ...
        assert_message_in_logs(logs[7], "gen_ai.tool.message", tool_message_1, spans[1])

        message = {
            "role": "assistant",
            "content": response_1.choices[0].message.content if expect_content else None,
        }
        choice = {
            "index": 0,
            "finish_reason": "stop",
...
def test_chat_completion_streaming(span_exporter, log_exporter, openai_client, instrument_with_content, vcr):
    latest_experimental_enabled = is_experimental_mode()
    kwargs = {
        "model": DEFAULT_MODEL,
        "messages": USER_ONLY_PROMPT,
...
def test_chat_completion_multiple_choices_streaming(
    ...
    latest_experimental_enabled = is_experimental_mode()
    with vcr.use_cassette("test_chat_completion_multiple_choices_streaming.yaml"):
        response_0 = openai_client.chat.completions.create(
    ...
    if latest_experimental_enabled:
        ...
        user_message = {"content": "What's the weather in Seattle and San Francisco today?"}
        assert_message_in_logs(logs[1], "gen_ai.user.message", user_message, spans[0])
...
def test_chat_completion_with_context_manager_streaming(
    ...
    latest_experimental_enabled = is_experimental_mode()
    with vcr.use_cassette("test_chat_completion_with_context_manager_streaming.yaml"):
...
def chat_completion_multiple_tools_streaming(
    ...
    if latest_experimental_enabled:
        ...
        system_message = {"content": WEATHER_TOOL_PROMPT[0]["content"]} if expect_content else None
        assert_message_in_logs(logs[0], "gen_ai.system.message", system_message, spans[0])

        user_message = {"content": "What's the weather in Seattle and San Francisco today?"} if expect_content else None
        assert_message_in_logs(logs[1], "gen_ai.user.message", user_message, spans[0])
