...
try:
    ...
    _create_params = set(inspect.signature(_AsyncResponses.create).parameters)
...
def test_async_responses_multiple_instrument_uninstrument_cycles(
...
async def test_async_responses_create_streaming_delegates_response_attribute(
    request, async_openai_client, instrument_no_content
):
    _skip_if_not_latest()
    skip_if_cassette_missing_and_no_real_key(request)
...
@pytest.mark.skipif(
    ...
    reason=(
        "openai SDK too old to support 'tools' parameter on AsyncResponses.create"
    ),
)
