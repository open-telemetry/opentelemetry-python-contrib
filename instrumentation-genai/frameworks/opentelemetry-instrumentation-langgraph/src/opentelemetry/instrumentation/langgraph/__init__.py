...
from opentelemetry.instrumentation.langgraph.patch import (
    ...
    create_async_node_execution_wrapper,
    create_async_route_wrapper,
    create_async_tool_node_wrapper,
    create_aupdate_state_wrapper,
    create_batch_wrapper,
    create_enhanced_ainvoke_wrapper,
    create_enhanced_invoke_wrapper,
    create_get_state_wrapper,
    create_invoke_wrapper,
    create_node_execution_wrapper,
    create_react_agent_wrapper,
    create_route_wrapper,
    create_stream_wrapper,
    create_task_execution_wrapper,
...
class LangGraphInstrumentor(BaseInstrumentor):
    ...
    def _instrument(self, **kwargs: Any) -> None:
        ...
        # Wrap Pregel methods (core execution) with enhanced wrappers
        wrap_function_wrapper(
            module="langgraph.pregel",
            name="Pregel.invoke",
            wrapper=create_enhanced_invoke_wrapper(tracer, workflow_duration),
        )

        wrap_function_wrapper(
            module="langgraph.pregel",
            name="Pregel.ainvoke",
            wrapper=create_enhanced_ainvoke_wrapper(tracer, workflow_duration),
        )
        ...
        # ================================================================
        # Enhanced Tracing: Node Execution and Routing
        # ================================================================

        # Wrap node execution for individual node tracing
        # IMPORTANT: We must wrap in BOTH _retry module AND _runner module
        # because _runner imports run_with_retry locally before our wrapper
        # can be applied. We need to patch both the source and the import.

        # Wrap in source module
        self._safe_wrap(
            "langgraph.pregel._retry",
            "run_with_retry",
            create_node_execution_wrapper(tracer),
        )

        self._safe_wrap(
            "langgraph.pregel._retry",
            "arun_with_retry",
            create_async_node_execution_wrapper(tracer),
        )

        # Also wrap the local references in _runner (imported copy)
        self._safe_wrap_module_attr(
        ...
        # Wrap conditional routing for decision visibility
        # This shows which branches are taken in conditional edges
        self._safe_wrap(
            "langgraph.graph._branch",
            "BranchSpec._route",
            create_route_wrapper(tracer),
        )

        self._safe_wrap(
            "langgraph.graph._branch",
            "BranchSpec._aroute",
            create_async_route_wrapper(tracer),
        )
    ...
    def _safe_wrap_module_attr(
        self, module: str, attr_name: str, wrapper: Any
    ) -> None:
        """Wrap a module-level attribute that was imported from elsewhere.

        This is needed when a module imports a function locally like:
            from other_module import func
        and we need to wrap the local reference, not just the original.

        Args:
            module: The module path containing the imported attribute.
            attr_name: The attribute name to wrap.
            wrapper: The wrapper function.
        """
        import sys
        ...
        try:
            ...
            # Create a new wrapper that calls our wrapper with the original
            def create_wrapped(orig):
                def wrapped(*args, **kwargs):
                    return wrapper(orig, None, args, kwargs)
                return wrapped

            # Replace the attribute
            setattr(mod, attr_name, create_wrapped(original))
        except (ImportError, AttributeError, ModuleNotFoundError) as e:
            logger.debug(
                "Could not wrap module attr %s.%s: %s",
                module,
                attr_name,
                e,
            )
    ...
    def _uninstrument(self, **kwargs: Any) -> None:  # noqa: ARG002
        ...
        # Unwrap node execution (from _retry module)
        try:
            from langgraph.pregel import _retry

            self._safe_unwrap(_retry, "run_with_retry")
            self._safe_unwrap(_retry, "arun_with_retry")
        except (ImportError, ModuleNotFoundError):
            pass

        # Unwrap conditional routing
        try:
            from langgraph.graph import _branch

            self._safe_unwrap(_branch.BranchSpec, "_route")
            self._safe_unwrap(_branch.BranchSpec, "_aroute")
        except (ImportError, ModuleNotFoundError):
            pass
