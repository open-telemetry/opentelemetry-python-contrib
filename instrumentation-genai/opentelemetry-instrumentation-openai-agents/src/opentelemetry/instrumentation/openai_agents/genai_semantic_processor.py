...
class GenAISemanticProcessor(TracingProcessor):
    ...
    def __init__(
        ...
        self._capture_system_instructions = os.getenv(
            "OTEL_GENAI_CAPTURE_SYSTEM_INSTRUCTIONS", "false"
        ).lower() == "true"
        self._capture_messages = os.getenv(
            "OTEL_GENAI_CAPTURE_MESSAGES", "false"
        ).lower() == "true"
        self._capture_tool_definitions = os.getenv(
            "OTEL_GENAI_CAPTURE_TOOL_DEFINITIONS", "false"
        ).lower() == "true"
        ...
        try:
            ...
            metric_attrs = {
                ...
                "gen_ai.request.model": (
                    attributes.get(GEN_AI_REQUEST_MODEL) or
    ...
    def _get_span_kind(self, span_data: Any) -> SpanKind:
        ...
        if isinstance(span_data, (GenerationSpanData, ResponseSpanData, 
                                  TranscriptionSpanData, SpeechSpanData)):
            return SpanKind.CLIENT  # API calls to model providers
        if isinstance(span_data, (AgentSpanData, GuardrailSpanData, HandoffSpanData)):
    ...
    def on_span_end(self, span: Span[Any]) -> None:
        ...
        if not (otel_span := self._otel_spans.pop(span.span_id, None)):
            ...
            try:
                ...
                logger.warning(
                    "Failed to extract attributes for span %s: %s", span.span_id, e
                )
            return
    ...
    def _get_attributes_from_generation_span_data(
        ...
        if hasattr(span_data, 'embedding_dimension'):
            yield GEN_AI_EMBEDDINGS_DIMENSION_COUNT, span_data.embedding_dimension
        ...
        # Output type
        yield GEN_AI_OUTPUT_TYPE, normalize_output_type(self._infer_output_type(span_data))
    ...
    def _get_attributes_from_agent_span_data(
        ...
        if self._capture_tool_definitions and hasattr(span_data, 'agent_definitions'):
            yield GEN_AI_ORCHESTRATOR_AGENT_DEFINITIONS, safe_json_dumps(
    ...
    def _get_attributes_from_function_span_data(
        ...
        if self._capture_tool_definitions and hasattr(span_data, 'tool_definitions'):
            yield GEN_AI_TOOL_DEFINITIONS, safe_json_dumps(span_data.tool_definitions)
    ...
    def _cleanup_spans_for_trace(self, trace_id: str) -> None:
        ...
        spans_to_remove = [
            span_id for span_id in self._otel_spans.keys()
