"""Weekend planner with OpenAI Agents + OpenTelemetry traces to Azure Application Insights.

Prereqs:
- pip install openai openai-agents opentelemetry-sdk azure-monitor-opentelemetry-exporter python-dotenv rich
- Set APPLICATION_INSIGHTS_CONNECTION_STRING in your environment
- For GitHub Models: set GITHUB_TOKEN (and optionally GITHUB_OPENAI_BASE_URL, GITHUB_MODEL)
- For Azure OpenAI: set AZURE_OPENAI_ENDPOINT, AZURE_OPENAI_VERSION, AZURE_OPENAI_CHAT_DEPLOYMENT
"""
...
def _set_capture_env(provider: str, base_url: str) -> None:
    """Set GenAI capture context (optional; capture defaults to enabled)."""
    os.environ.setdefault("OTEL_GENAI_AGENT_NAME", "Weekend Planner Agent")
    os.environ.setdefault(
        "OTEL_GENAI_AGENT_DESCRIPTION",
        "Assistant that plans weekend activities using weather and events data",
    )
    os.environ.setdefault("OTEL_GENAI_AGENT_ID", "weekend-planner")
...
def _configure_otel() -> None:
    """Configure tracer provider to export to both Console and Azure Monitor.

    - Always adds ConsoleSpanExporter for local visibility.
    - Adds AzureMonitorTraceExporter when APPLICATION_INSIGHTS_CONNECTION_STRING is set
      and the azure-monitor-opentelemetry-exporter package is installed.
    """
    ...
    # Always enable console exporter
    ...
    if conn:
        try:
            ...
            print(
                "[otel] Azure Monitor exporter not installed. "
        ...
        print(
            "[otel] APPLICATION_INSIGHTS_CONNECTION_STRING not set; skipping Azure Monitor"
        )
...
async def main() -> None:
    ...
    with tracer.start_as_current_span(
        ...
        user_request = "What can I do this weekend in Seattle?"
