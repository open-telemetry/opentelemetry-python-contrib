OpenTelemetry LangChain + LangGraph Workflow Example
===================================================

This example shows how to instrument a LangGraph StateGraph when configuring
OpenTelemetry SDK and instrumentations manually. The graph has a single node
that calls ChatOpenAI; the LangChain instrumentor traces those LLM calls.

When `main.py <main.py>`_ is run, it exports traces to an OTLP-compatible endpoint.
Traces include details such as the span name and other attributes for the LLM
invocations inside the graph node.
...
You should see the capital of France generated by the LangGraph LLM node while traces export to your configured observability tool.
