...
"""
IBM watsonx.ai client instrumentation supporting `ibm_watsonx_ai`, it can be
enabled by using ``WatsonxInstrumentor``.

.. _ibm_watsonx_ai: https://pypi.org/project/ibm-watsonx-ai/
...
A custom ``CompletionHook`` implementation can also be passed programmatically::
    ...
    WatsonxInstrumentor().instrument(completion_hook=my_hook)
...
from opentelemetry.instrumentation.watsonx.package import _instruments
from opentelemetry.util.genai.completion_hook import load_completion_hook
from opentelemetry.util.genai.handler import TelemetryHandler

from .patch import chat, generate, generate_text_stream

_MODULE = "ibm_watsonx_ai.foundation_models"
...
class WatsonxInstrumentor(BaseInstrumentor):
    ...
    def _instrument(self, **kwargs: Any) -> None:
        """Enable IBM watsonx.ai instrumentation."""
        ...
        wrap_function_wrapper(
            ...
            generate_text_stream(handler),
        )

    def _uninstrument(self, **kwargs: Any) -> None:
        from ibm_watsonx_ai.foundation_models import (  # pylint: disable=import-outside-toplevel  # noqa: PLC0415
            ModelInference,
        )

        unwrap(ModelInference, "generate")
        unwrap(ModelInference, "chat")
        unwrap(ModelInference, "generate_text_stream")
