...
@pytest.fixture(scope="function")
def instrument_metadata_mode(tracer_provider, logger_provider, meter_provider):
    """Instrument with METADATA capture mode (default - only metadata, no content/events)."""
    os.environ.update({OTEL_INSTRUMENTATION_GENAI_CAPTURE_MODE: "metadata"})
    ...
    yield instrumentor
    os.environ.pop(OTEL_INSTRUMENTATION_GENAI_CAPTURE_MODE, None)
    instrumentor.uninstrument()


@pytest.fixture(scope="function")
def instrument_all_mode(tracer_provider, logger_provider, meter_provider):
    """Instrument with ALL capture mode (full content and events)."""
    os.environ.update({OTEL_INSTRUMENTATION_GENAI_CAPTURE_MODE: "all"})
...
@pytest.fixture(scope="function")
def instrument_default_mode(tracer_provider, logger_provider, meter_provider):
    """Instrument with default capture mode (metadata - no env var set)."""
    # Clear any existing capture mode env vars
    os.environ.pop(OTEL_INSTRUMENTATION_GENAI_CAPTURE_MODE, None)
    os.environ.pop(OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT, None)
