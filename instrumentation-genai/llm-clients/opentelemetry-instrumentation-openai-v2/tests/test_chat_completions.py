...
def test_chat_completion_handles_not_given(
    ...
    """Test that NOT_GIVEN parameters are handled properly in METADATA mode.

    When OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT=False is set,
    the instrumentation uses METADATA capture mode which skips log events.
    """
    ...
    # In METADATA mode, log events are skipped
    logs = log_exporter.get_finished_logs()
    assert len(logs) == 0
...
def test_chat_completion_no_content(
    ...
    """Test that in METADATA mode (no content capture), log events are skipped.

    When OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT=False is set,
    the instrumentation uses METADATA capture mode which:
    - Still captures spans with metadata (token counts, model, etc.)
    - Does NOT emit log events
...
def chat_completion_tool_call(
    ...
    logs = log_exporter.get_finished_logs()
    # In METADATA mode (no content capture), log events are skipped entirely
