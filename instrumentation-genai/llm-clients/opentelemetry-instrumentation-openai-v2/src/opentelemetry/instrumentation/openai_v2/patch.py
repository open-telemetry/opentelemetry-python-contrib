    ...
    def traced_method(wrapped, instance, args, kwargs):
        ...
        with tracer.start_as_current_span(
            ...
            if span.is_recording():
                set_tools_attributes(span, kwargs, capture_mode)
            ...
            # Only emit log events in ALL mode
            if capture_mode == CaptureMode.ALL:
                for message in kwargs.get("messages", []):
                    logger.emit(message_to_event(message, True))
            ...
            try:
                ...
                if span.is_recording():
                    _set_response_attributes(
                        span, parsed_result, logger, capture_mode
                    )
                    # Set response content on span
                    set_response_content_on_span(span, parsed_result, capture_mode)

                # Only emit log events in ALL mode
                if capture_mode == CaptureMode.ALL:
                    for choice in getattr(parsed_result, "choices", []):
                        logger.emit(choice_to_event(choice, True))
            ...
            try:
                ...
                if span.is_recording():
                    _set_embeddings_response_attributes(
                        span, result, capture_mode, input_text
                    )
...
def _set_response_attributes(
    span, result, logger: Logger, capture_mode: CaptureMode
):
...
class StreamWrapper:
    ...
    def __init__(
        ...
        # Derive boolean for internal content buffering
        self.capture_content = capture_mode == CaptureMode.ALL
        self.logger = logger
    ...
    def cleanup(self):
        if self._span_started:
            ...
            # Only emit log events in ALL mode
            if self.capture_mode == CaptureMode.ALL:
