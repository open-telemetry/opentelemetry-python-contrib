...
"""Span-emitting wrappers for Milvus client operations.
...
# There is no ``DbSystemNameValues`` enum member for Milvus, so we use the
# literal value recommended by the semantic conventions for otherwise-unlisted
# database systems.
_DB_SYSTEM_MILVUS = "milvus"


def _collection_name_from_args(
    args: tuple[Any, ...], kwargs: dict[str, Any]
) -> str | None:
    """Extract the collection name from a Milvus client call.

    Every instrumented ``MilvusClient`` method accepts the collection name as
    its first positional argument or as the ``collection_name`` keyword
    argument.
    """
...
def _set_attributes(
    ...
    span.set_attribute(DB_SYSTEM_NAME, _DB_SYSTEM_MILVUS)
