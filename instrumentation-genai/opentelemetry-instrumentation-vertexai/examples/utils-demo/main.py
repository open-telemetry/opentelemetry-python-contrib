# pylint: skip-file
"""
Vertex AI Agent Engine — invoke_agent instrumentation demo.

Combines automatic instrumentation (``VertexAIInstrumentor``) with
manual ``TelemetryHandler.agent()`` spans from
...
def main():
    ...
    # Vertex AI GenerativeModel (uses ADC, auto-instrumented by VertexAIInstrumentor)
    model = GenerativeModel(MODEL)
    handler = TelemetryHandler()

    # ----- Step 1: LLM call inside invoke_agent span -----
    # The generate_content call is made inside the agent context so the
    # auto-instrumented chat span appears as a child of invoke_agent.
    with handler.agent(
        AgentInvocation(
            agent_name=AGENT_NAME,
            provider="gcp_vertex_ai",
            request_model=MODEL,
            agent_description="LLM call wrapped in invoke_agent span",
            server_address=f"{GCP_LOCATION}-aiplatform.googleapis.com",
        )
    ) as _invocation:
        print("[invoke_agent] Generating content...")
    ...
    print(f"\n[create_agent] Local agent created: {AGENT_NAME}")

    # ----- Step 3: Deploy to Vertex AI Agent Engine (invoke_agent span) -----
