...
# There is no groq-specific `gen_ai.system` / `gen_ai.provider.name` attribute
# module in semconv (unlike ``openai_attributes``). Groq is covered by the
# well-known value ``groq`` in ``GenAiProviderNameValues`` / ``GenAiSystemValues``.
...
def non_numerical_value_is_set(value: bool | str | NotGiven | None) -> bool:
...
def _groq_response_format_to_output_type(response_format_type: str) -> str:
...
def get_llm_request_attributes(
    ...
    if operation_name == GenAIAttributes.GenAiOperationNameValues.CHAT.value:
        ...
        if (response_format := kwargs.get("response_format")) is not None:
            # response_format may be string or object with a string in `type`
            if isinstance(response_format, Mapping):
                if (
                    response_format_type := response_format.get("type")
                ) is not None:
                    attributes[GenAIAttributes.GEN_AI_OUTPUT_TYPE] = (
                        _groq_response_format_to_output_type(
                            response_format_type
                        )
                    )
            elif isinstance(response_format, str):
                attributes[GenAIAttributes.GEN_AI_OUTPUT_TYPE] = (
                    _groq_response_format_to_output_type(response_format)
                )
        ...
        if isinstance(response_format, Mapping):
            if (
                ...
                invocation.attributes[GenAIAttributes.GEN_AI_OUTPUT_TYPE] = (
                    _groq_response_format_to_output_type(response_format_type)
                )
        elif isinstance(response_format, str):
            invocation.attributes[GenAIAttributes.GEN_AI_OUTPUT_TYPE] = (
                _groq_response_format_to_output_type(response_format)
