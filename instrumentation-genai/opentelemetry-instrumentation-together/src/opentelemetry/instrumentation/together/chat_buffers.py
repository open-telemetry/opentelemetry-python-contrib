...
class ChoiceBuffer:
    ...
    def append_tool_call(self, tool_call: Any) -> None:
        # together ``ChoiceDelta.tool_calls`` items expose ``index``, ``id`` and
        # a ``function`` with ``name`` / ``arguments`` (same shape as OpenAI's
        # streaming tool-call deltas). ``index`` may be a float in the SDK types,
        # so coerce to int before using it as a list position.
