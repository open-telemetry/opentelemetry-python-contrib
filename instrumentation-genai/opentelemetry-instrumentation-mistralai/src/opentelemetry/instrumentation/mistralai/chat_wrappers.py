...
def _chunk_from_event(event: Any) -> Any:
    """mistralai streams yield ``CompletionEvent`` objects that carry the actual
    ``CompletionChunk`` under ``.data``. Fall back to the object itself for
...
class _ChatStreamMixin:
    ...
    def _process_chunk(self, event: Any) -> None:
...
class ChatStreamWrapper(
    ...
    def close(self) -> None:
        # mistralai's ``EventStream`` does not expose ``close()``; the underlying
        # httpx response owns the connection instead.
        try:
            self._self_stream.response.close()
...
class AsyncChatStreamWrapper(
    ...
    async def close(self) -> None:
        # mistralai's ``EventStreamAsync`` does not expose ``close()``; the
        # underlying httpx response owns the connection instead.
        try:
            await self._self_stream.response.aclose()
        except Exception as error:  # pylint: disable=broad-exception-caught
            self._safe_finalize_failure(error)
            raise
        self._safe_finalize_success()
