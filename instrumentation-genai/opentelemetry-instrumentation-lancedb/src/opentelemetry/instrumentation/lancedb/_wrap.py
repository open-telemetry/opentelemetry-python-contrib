...
"""Span-emitting wrappers for LanceDB table operations.
...
# There is no ``DbSystemNameValues`` enum member for LanceDB, so we use the
# literal value recommended by the semantic conventions for otherwise-unlisted
# database systems.
_DB_SYSTEM_LANCEDB = "lancedb"


def _collection_name_from_instance(instance: Any) -> str | None:
    """Extract the table (collection) name from a ``LanceTable`` instance.

    Unlike some clients, LanceDB carries the table name on the instance being
    called (``LanceTable.name``) rather than as a call argument.
    """
...
def _set_attributes(
    ...
    span.set_attribute(DB_SYSTEM_NAME, _DB_SYSTEM_LANCEDB)
