...
"""Span-emitting wrappers for Weaviate collection-management operations.
...
number of objects, ``limit`` / ``top_k``, query vectors, or filters) are not
...
# There is no ``DbSystemNameValues`` enum member for Weaviate, so we use the
# literal value recommended by the semantic conventions for otherwise-unlisted
# database systems.
_DB_SYSTEM_WEAVIATE = "weaviate"


def _collection_name_from_args(
    args: tuple[Any, ...], kwargs: dict[str, Any]
) -> str | None:
    """Extract the collection name from a Weaviate ``_Collections`` call.

    The instrumented ``create`` / ``get`` / ``delete`` / ``exists`` /
    ``create_from_dict`` methods accept the collection name as their first
    positional argument or as the ``name`` keyword argument. Operations such as
    ``delete_all`` and ``list_all`` have no single collection name, so ``None``
...
def _set_attributes(
    ...
    span.set_attribute(DB_SYSTEM_NAME, _DB_SYSTEM_WEAVIATE)
