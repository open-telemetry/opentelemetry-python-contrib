...
# https://github.com/anthropics/anthropic-sdk-python/blob/9b5ab24ba17bcd5e762e5a5fd69bb3c17b100aaa/src/anthropic/resources/messages/messages.py#L2080
# https://github.com/anthropics/anthropic-sdk-python/blob/9b5ab24ba17bcd5e762e5a5fd69bb3c17b100aaa/src/anthropic/resources/messages/messages.py#L2147
...
class AsyncStreamWrapper(AsyncIterator["RawMessageStreamEvent"]):
    """Wrapper for async Anthropic Stream that handles telemetry.

    This wrapper wraps AsyncStream[RawMessageStreamEvent] returned by
    AsyncMessages.create(stream=True). It processes streaming chunks,
    ...
    def _process_chunk(self, chunk: "RawMessageStreamEvent") -> None:
        ...
        # Handle message_delta event - contains stop_reason and output token usage
            ...
            # message_delta contains usage with output tokens and may repeat input/cache tokens
...
class AsyncMessageStreamManagerWrapper:
    ...
    This wrapper wraps the AsyncMessageStreamManager async context manager returned by
    AsyncMessages.stream(). It extracts telemetry data from the final message
    ...
    def __init__(
        ...
        self._handler = handler
        self._invocation = invocation
        self._message_stream: Optional["AsyncMessageStream"] = None

    async def __aenter__(self) -> "AsyncMessageStream":
        """Enter the async context and return the underlying AsyncMessageStream."""
        try:
            self._message_stream = await self._stream_manager.__aenter__()
    ...
    async def __aexit__(
        ...
        if self._message_stream is not None and exc_type is None:
            await self._extract_telemetry_from_stream()
            self._handler.stop_llm(self._invocation)
    ...
    async def _extract_telemetry_from_stream(self) -> None:
        ...
        try:
            ...
            final_message = await self._message_stream.get_final_message()
