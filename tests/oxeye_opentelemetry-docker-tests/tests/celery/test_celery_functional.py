...
import oxeye_opentelemetry.instrumentation.celery
...
from oxeye_opentelemetry.sdk.trace import TracerProvider, export
...
def test_instrumentation_info(celery_app, memory_exporter):
    ...
    assert async_span.instrumentation_info.name == "apply_async/{0}".format(
        oxeye_opentelemetry.instrumentation.celery.__name__
    )
    assert async_span.instrumentation_info.version == "apply_async/{0}".format(
        oxeye_opentelemetry.instrumentation.celery.__version__
    )
