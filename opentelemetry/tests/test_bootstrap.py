...
from opentelemetry.cmd.instrument import bootstrap
from opentelemetry.cmd.instrument.bootstrap_gen import libraries
...
class TestBootstrap(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.pkg_patcher = patch(
            "opentelemetry.cmd.instrument.bootstrap._find_installed_libraries",
            return_value=cls.installed_libraries,
        )

        cls.pip_install_patcher = patch(
            "opentelemetry.cmd.instrument.bootstrap._sys_pip_install",
        )
        cls.pip_check_patcher = patch(
            "opentelemetry.cmd.instrument.bootstrap._pip_check",
        )
