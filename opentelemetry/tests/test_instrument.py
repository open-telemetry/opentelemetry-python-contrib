...
from opentelemetry.cmd.instrument import instrument
from opentelemetry.environment_variables import OTEL_TRACES_EXPORTER


class TestRun(TestCase):
    auto_instrumentation_path = join(
        dirname(abspath(instrument.__file__)), "site"
    )

    @classmethod
    def setUpClass(cls):
        cls.execl_patcher = patch(
            "opentelemetry.cmd.instrument.instrument.execl"
        )
        cls.which_patcher = patch(
            "opentelemetry.cmd.instrument.instrument.which"
        )
...
class TestExecl(TestCase):
    ...
    @patch("opentelemetry.cmd.instrument.instrument.which")
    @patch("opentelemetry.cmd.instrument.instrument.execl")
    def test_execl(
