...
from opentelemetry.conf import configure_tracing
...
class TestConf(unittest.TestCase):
    ...
    def setUp(self):
        self._default_exporter_patcher = patch(
            "opentelemetry.conf.components._DEFAULT_SPAN_EXPORTER", "console",
        )
        self._default_exporter_patcher.start()
        self._original_provider = trace._TRACER_PROVIDER
        return super().setUp()

    def tearDown(self) -> None:
        trace._TRACER_PROVIDER = self._original_provider
        self._default_exporter_patcher.stop()
