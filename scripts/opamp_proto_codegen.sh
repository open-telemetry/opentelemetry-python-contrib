...
# tag in the opamp-spec repo that you want to build from. Then, just run
# this script to update the proto files. Commit the changes as well as any
# fixes needed in the OpAMP client.
#
# Optional envars:
...
buf generate "$OPAMP_SPEC_REPO_DIR/proto" \
    --template '{"version":"v2","plugins":[{"local":"protoc-gen-py","out":".","opt":["init_files=false"]}]}'
